...
// *****************************Throttle & Debounce***************************** \\
// Пояснити принцип роботи throttle та debounce, показати можливості налаштування за допомогою параметрів leading/trailing
...
// // Делегування та перевірка на те чи клітинка порожня
//     if (evt.target === evt.currentTarget || evt.target.textContent) {
//         return;
//     }

// // Отримуємо id клітинки по якій клікнули
//     const id = Number(evt.target.dataset.id);
//     let isWinner = false;

//     if (player === 'X') {
// // Додаємо хід гравця Х до його історії
//         historyX.push(id);
// // Викликаємо функцію checkWinner тільки в тому випадку якщо один з гравців зробив три або більше ходів
//         isWinner = historyX.length >= 3 ? checkWinner(historyX) : false;
//     } else {
// // Додаємо хід гравця О до його історії
//         historyO.push(id);
// // Викликаємо функцію checkWinner тільки в тому випадку якщо один з гравців зробив три або більше ходів
...
//     if (isWinner) {
// // Створюємо модальне вікно для відображення переможця
...
// // Перевіряємо чи хоча би одна виграшна комбінація містить в собі всі ходи гравця (Х/О)
